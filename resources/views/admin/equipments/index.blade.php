            ...
            <div class="card">
                <div class="card-header">
                    <button onclick="addEquipment()"
                            class="btn btn-info">{{__('admin.equipment.heading_create')}}</button>
                        ...
                        <tbody>
                            @foreach($models as $model)
                                <tr>
                                    <td>{{$model->name}}</td>
                                    <td>
                                        <form action="{{route('admin.equipment-types.delete', $model->id)}}"
                                              method="post">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{route('admin.equip_type', ['equipment' => $model])}}"
                                               class="btn btn-outline-info mr-2">
                                                {{__('admin.equipment.equip_types')}}
                                            </a>
                                            <button type="button"
                                                    onclick="edit('{{route('admin.equipment-types.update', $model->id)}}', '{{$model->name}}')"
                            ...
                            <form action="{{route('admin.equipment-types.store')}}" method="POST"
                                ...
                                <div class="d-flex justify-content-between">
                                    <button type="submit" id="submit"
                                            class="btn btn-primary">{{__('global.btn_add')}}</button>
                                    <button type="button" class="btn btn-default"
                                            data-dismiss="modal">{{__('global.btn_cancel')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@section('js')
    ...
    <script>
        ...
        function addEquipment() {
            ...
            $('#form').attr('action', '{{route('admin.equipment-types.store')}}');
        }
